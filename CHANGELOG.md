...
### Changed
- **`pom-core` plugin manifest version bumped from `0.1.0` to `0.3.0`.** Reflects the additive surface changes shipped across v0.2 (4 new skills: `pom-explain`, `pom-pulse`, `pom-render`, `pom-sync-ado`; `pom-status --since` mode; ADO branch/PR emission from `pom-promote-to-backlog`; paste-ready stakeholder blocks) and v0.3 (`pom-council` skill, `pom-council-synthesizer` agent, `pom-trio council_mode`, validator R3.11/R3.12). The matching `pom-core` entry in `.claude-plugin/marketplace.json` is bumped in lockstep. No breaking changes; consumers pinning to `v0.1.0` continue to work — this aligns metadata with reality. Industry overlays (`pom-insurance`, `pom-fintech`, `pom-healthcare`, `pom-retail`) and the top-level `marketplace.json` version stay at `0.1.0` — zero post-v0.1.0 changes, and the plugin list itself is unchanged.
